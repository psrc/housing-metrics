...
opp_index_column <- static_column_chart(t= acs_low_verylow,
                                        x = "race_ethnicity", y = "count",
                                        fill = "opp_index",
                                        pos = "dodge",
                                        moe = "count_moe",
...
opp_index_column <- static_column_chart(t= acs_race_totals,
                                        x = "race_ethnicity", y = "count",
                                        fill = "race_ethnicity",
                                        pos = "dodge",
                                        moe = "count_moe",
...
# 4. create a subset table that calculates: the total count of those in the Low/Very Low indices
acs_race_opp_low <- acs_low_verylow %>%
  group_by(opp_index)%>%
  summarise(count_opp=sum(count))
...
acs_comb_low_verylow <- acs_comb_low_verylow %>%
  add_column(total_opp_index = rep(c(896873, 850560, 896873, 850560, 896873, 850560, 896873, 850560, 896873, 850560, 896873, 850560, 
                                     896873, 850560)), 
             .after = 'share')

# 5. calculate share_moe
acs_comb_low_verylow$share_moe <- acs_comb_low_verylow$count_moe/acs_comb_low_verylow$total_opp_index

# 6. rearrange
acs_comb_low_verylow <- acs_comb_low_verylow[c(1, 2, 3, 4, 6, 8, 5, 7)]%>%
  mutate(opp_index = fct_relevel(opp_index, "Very Low", "Low"))
