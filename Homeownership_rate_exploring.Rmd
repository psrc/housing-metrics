...
tenure_all_func <- function(year){
  ...
  # Obtain the data
  pums_raw <- get_psrc_pums(5, year, "h", c("COUNTY", "TEN","POVPIP","PRACE", "R18", "R65", "LNGI", "HDIS"))
  ...
  # Analysis --------------
  tenure_inc_re <- psrc_pums_count(pums_new_vars, group_vars = c("COUNTY", "tenure", "income_bin", "PRACE", "Youth_cat", "Older_cat", "LEP_cat",
  ...
  tenure_inc_re_region <- psrc_pums_count(pums_new_vars, group_vars = c("tenure", "income_bin", "PRACE", "Youth_cat", "Older_cat", "LEP_cat",
                                                                        "Disability_cat"),rr=TRUE)%>%
    filter(tenure == 'owner')%>%
    filter(PRACE != "Total") %>%
    filter(income_bin  != "Total") %>%
