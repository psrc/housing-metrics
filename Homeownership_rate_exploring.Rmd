...
tenure_all_func <- function(year){
  ...
  # Obtain the data
  #setwd("C:/Users/MGrzybowski/Downloads")
  pums_raw <- get_psrc_pums(5, year, "h", c("COUNTY", "TEN","POVPIP","PRACE", "R18", "R65", "LNGI"))
  #setwd("Y:/Equity Indicators/experimenting/R-scripts/PUMS/Homeownership_exploring_files")
...
# Analysis --------------
  tenure_all <- psrc_pums_count(pums_new_vars, group_vars = c("COUNTY", "tenure", "income_bin", "PRACE", "Youth_cat", "Older_cat", "LEP_cat"),rr=TRUE) %>%
    filter(tenure == "owner") %>%
    filter(PRACE != "Total") %>%
    filter(income_binyea  != "Total") %>%
...
```{r}
just_region_11 <- tenure_all %>% 
  filter(COUNTY== "King") %>% 
  filter(DATA_YEAR==2011) %>% 
  arrange(PRACE, tenure)
just_region_16 <- tenure_all %>% 
  filter(COUNTY=="King") %>% 
  filter(DATA_YEAR==2016) %>% 
  arrange(PRACE, tenure)
just_region_21 <- tenure_all %>% 
  filter(COUNTY=="King") %>% 
