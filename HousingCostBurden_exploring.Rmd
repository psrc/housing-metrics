...
data_indicator <- pums_ospi_elmer %>% 
  mutate(indicator_attribute = case_when(indicator_attribute == "Greater than 50 percent" |
                                          indicator_attribute == "Between 30 and 50 percent" ~ "Cost Burdened (More than or Equal to 30 percent)",
...
```
<span style="color: #00A7A0">
The disability category and LEP is missing all data in 2011.
</span> 
\
\
...
```{r}
num_yr_subgrp <- num_county*num_indatt #20
```
...
```
<span style="color: #00A7A0">
The disability and LEP categories are missing all data in 2011.
</span> 
\
\
...
```
<span style="color: #00A7A0">
All data in 2011 is missing 20 entries.
</span> 
\
\
...
\
\
These charts also help to confirm that the shares add up to 100% - only relevant when *indicator_attribute* has more than one category. The *indicator_attribute* for rent cost burden includes 2 categories: "More than 30 percent" and "Less than 30 percent" 
\
\
...
qplot(data_year_yr, fact_value, 
      ...
      data = data_clean_over, 
      main = paste0(indicator_title, ": Region, more than or equal to 30 percent")) + theme(axis.title.x = element_blank(),
                                                          axis.title.y = element_blank()) + ylim(0, 1)

qplot(data_year_yr, fact_value, 
      colour = county, shape = focus_type_ord, 
      data = data_clean_lessthirty, 
...
data_clean11 <- data_clean_over %>% 
  filter(data_year==2011)
data_clean16 <- data_clean_over %>% 
  filter(data_year==2016)
data_clean21 <- data_clean_over %>% 
  filter(data_year==2021)
```
...
facet_by_geography <- function(geography, focus.text){
  # filter data
  data_clean_geo <- data_clean_over %>% 
  ...
  static_facet_column_chart(t=data_clean_geo,
                            ...
                            subtitle="Greater than or equal to 30 percent spent on rent",
                            source="U.S. Census Bureau, American Community Survey (ACS) 5-Year Public Use Microdata Sample (PUMS)")
}
```
...
dif_groups <- function(geography, group, category){
  region_btgrp_dif <- data_clean_over %>% 
...
dif_equity <- static_facet_line_chart(t=dif_equity_groups,
                                      ...
                                      subtitle="Greater than or equal to 30 percent",
...
equity_group_facet_line <- function(focus.type, focus.text){
  # filter data
  data_clean_filter <- data_clean_over %>% 
  ...
  static_facet_line_chart(t=data_clean_filter,
                          ...
                          subtitle="Greater than or equal to 30 percent",
                          source="U.S. Census Bureau, American Community Survey (ACS) 5-Year Public Use Microdata Sample (PUMS)") + 
    ggplot2::scale_y_continuous(limits = c(0, 0.90))+
...
p <- ggplot(data_clean_df, aes(fact_value, focus_attribute_ord)) +
  ...
  labs(x="Percentage of adults", #following labs will need to be adjusted for indicator
       ...
       subtitle = "Greater than 30 percent for households",
...
```{r}
data_clean_region <- data_clean_over %>% 
  dplyr::filter(county=="Region")
