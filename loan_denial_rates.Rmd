...
```{r, Helper attributes / functions from Michael, eval = FALSE}
...
```{r, create columns for specific charts/plots, plot 1}

# chart 1 - Loan Denial Rates by Race and Ethnicity (2021)
...
```{r, create columns for specific charts/plots, plot 2}

# chart 2 - Time Series Loan Denial Rates by Race and Ethnicity (2018-2021)

region_POC <- region_clean %>% 
  mutate(POC = case_when(race_ethnicity == "White" ~"Non POC",
         TRUE ~ "POC")) %>%
  group_by(data_year, POC)%>%
  summarise(received=sum(received),
            denied=(sum(denied)))

region_POC$data_year <- as.factor(region_POC$data_year)

region_POC_calculate <- region_POC %>%
  mutate(share_denials_race = denied/received)

#first_col_vals <-  c("less than 50%", "50-79%", "80-99%", "100-119%", "120% or more")

# arranged by factor order
#region_POC_reorder <- region_POC_calculate %>% 
 # mutate(median_income = factor(median_income, levels = first_col_vals)) %>% 
  #arrange(median_income)

# time series line chart

denial_race_line_chart <- static_line_chart(t= region_POC_calculate,
                                            ...
                                            lwidth = 1,
                                            color = "psrc_pairs",
                                            title = "Loan Denial Rates by Race and Ethnicity (2018-2021)",
                                            subtitle = "Federal Financial Institutions Examination Council (FFIEC), Home Mortgage Disclosure Act (HMDA) Database")


denial_race_line_chart

denial_race_line_chart <- interactive_line_chart(t= region_POC_calculate,
                                            x = "data_year", y = "share_denials_race",
                                            fill = "POC",
                                            est = "percent",
                                            breaks = NULL,
...
# chart 3 - Loan Denial Rates by Income and Race/Ethnicity (2021)
...
```{r, create columns for specific charts/plots}
