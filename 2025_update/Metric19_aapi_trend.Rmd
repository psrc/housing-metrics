...
renter <- psrc_pums_count(df_pums_renter_aapi, group_vars=c("PRACE","RAC2P23_aapi_group10","rent_pct_income_30"), rr=TRUE) %>% 
    add_row(psrc_pums_count(df_pums_renter, group_vars=c("PRACE", "rent_pct_income_30")) %>% 
               mutate(RAC2P23_aapi_group10 = "Asian alone", .after="COUNTY")) %>% 
    filter(PRACE == "Asian alone") %>% 
    add_row(psrc_pums_count(df_pums_renter, group_vars=c("rent_pct_income_30")) %>%
           mutate(RAC2P23_aapi_group10 = "Region", .after="COUNTY"))
...
cost_burdened <- renter %>% 
  ...
  select(RAC2P23_aapi_group10, count, count_moe, share, share_moe, reliability) %>% 
  group_by(RAC2P23_aapi_group10) %>%
...
ggplot(cost_burdened, aes(x=reorder(RAC2P23_aapi_group10, share), y=share, fill=RAC2P23_aapi_group10)) +
  geom_col()+
...
job3_by_asian_alone <- psrc_pums_count(df_pums_p_aapi_renter_worker %>% filter(RAC2P23_aapi_group10_household!="Native Hawaiian and Other Pacific Islander"), group_vars=c("SOCP3")) %>%
  arrange(desc(count))


job3_by_aapi_race <- psrc_pums_count(df_pums_p_aapi_renter_worker, group_vars=c("RAC2P23_aapi_group10_household","SOCP3")) %>%
  mutate(PRACE="Asian alone", .before="RAC2P23_aapi_group10_household") %>% 
  add_row(psrc_pums_count(df_pums_renter_all_race, group_vars=c("PRACE", "SOCP3")) %>%
            mutate(RAC2P23_aapi_group10_household = "Asian alone", .after="COUNTY")) %>% 
  filter(PRACE == "Asian alone") %>% 
  add_row(psrc_pums_count(df_pums, group_vars=c("SOCP3")) %>%
           mutate(RAC2P23_aapi_group10_household = "Region", .after="COUNTY"))

job_df <- job3_by_aapi_race %>% 
  group_by(RAC2P23_aapi_group10_household) %>% 
  filter(SOCP3!="Total") %>%
  slice_max(count, n= 5) %>%
  mutate(occupation = str_c("Occupation_", row_number())) %>% 
  spread(occupation, SOCP3) %>% 
  select(RAC2P23_aapi_group10_household, 'Occupation_1', 'Occupation_2', 'Occupation_3', 'Occupation_4', 'Occupation_5')   %>%
...
job_df <- job_df[, lapply(.SD, paste0, collapse=""), by=RAC2P23_aapi_group10_household] %>% 
...
cost_burdened2 <- renter %>% 
  ...
  select(RAC2P23_aapi_group10, Percentage) %>% 
