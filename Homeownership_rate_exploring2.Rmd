---
title: "Homeownership_rate_exploring2"
...
indicator_title <- 'Housing Tenure'
```
...
data_indicator <- pums_ospi_elmer %>% 
 # dplyr::filter(indicator_attribute == "Owned")%>%
...
\
\
These charts also help to confirm that the shares add up to 100% - only relevant when *indicator_attribute* has more than one category. The *indicator_attribute* for rent cost burden includes 2 categories: "Owns" and "Rented" 
\
\
...
data_clean_rent <- data_clean %>% 
  filter(indicator_attribute=="Rented")
data_clean_own <- data_clean %>% 
  filter(indicator_attribute=="Owned")
```
...
qplot(data_year_yr, fact_value, 
      ...
      data = data_clean_rent, 
      main = paste0(indicator_title, ": Region, rented")) + theme(axis.title.x = element_blank(),
                                                          axis.title.y = element_blank()) + ylim(0, 1)

qplot(data_year_yr, fact_value, 
      colour = county, shape = focus_type_ord, 
      data = data_clean_own, 
      main = paste0(indicator_title, ": Region, owned")) + theme(axis.title.x = element_blank(),
                                                          axis.title.y = element_blank()) + ylim(0, 1)
```
...
data_clean11 <- data_clean_own %>% 
  filter(data_year==2011)
data_clean16 <- data_clean_own %>% 
  filter(data_year==2016)
data_clean21 <- data_clean_own %>% 
  filter(data_year==2021)
```
...
facet_by_geography <- function(geography, focus.text){
  ...
  static_facet_column_chart(t=data_clean_geo,
                            ...
                            scales="fixed",
                            ncol=3,
                            title=paste0("Homeownership Rates", focus.text),
                            subtitle="Owns",
                            source="U.S. Census Bureau, American Community Survey (ACS) 5-Year Public Use Microdata Sample (PUMS)")
}
```
...
dif_groups <- function(geography, group, category){
  region_btgrp_dif <- data_clean_own %>% 
...
dif_equity <- static_facet_line_chart(t=dif_equity_groups,
                                      ...
                                      scales="fixed",
                                      ncol=3,
                                      title=paste0("Homeownership Rates"),
                                      subtitle="owns",
...
equity_group_facet_line <- function(focus.type, focus.text){
  # filter data
  data_clean_filter <- data_clean_own %>% 
...
p <- ggplot(data_clean_df, aes(fact_value, focus_attribute_ord)) +
  ...
  labs(x="Percentage of adults", #following labs will need to be adjusted for indicator
       ...
       colour= "Year",
       shape="Year",
       title = paste0("Homeownership Rates", ": Region"),
       subtitle = "owns",
...
```{r}
data_clean_region <- data_clean_own %>% 
  dplyr::filter(county=="Region")
...
line_chart <- static_line_chart(t=data_clean_region,
                                ...
                                lwidth = 1,
                                color = "psrc_pairs",
                                title = "Homeownership Rates: Region",
                                subtitle = "owns",
                                source = "U.S. Census Bureau, American Community Survey (ACS) 5-Year Public Use Microdata Sample (PUMS)")+
    scale_y_continuous(limits = c(0, 0.85),
                       labels = scales::percent)
