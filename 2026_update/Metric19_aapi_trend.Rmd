...
```{r, include=FALSE}
library(here)
source(here("2026_update", "metric19_aapi_trend.R"))
```
...
renter <- psrc_pums_count(df_pums_renter_aapi, group_vars=c("PRACE","RAC2P_equivalent_aapi_group10","rent_pct_income_30"), rr=TRUE) %>% 
    add_row(psrc_pums_count(df_pums_renter, group_vars=c("PRACE", "rent_pct_income_30")) %>% 
               mutate(RAC2P_equivalent_aapi_group10 = "Asian", .after="COUNTY")) %>% 
    filter(PRACE == "Asian") %>% 
...
Most common jobs by in the region and the share of Asian populations within those jobs filter for renter cost burden before their occupation
...
#  mutate(PRACE="Asian", .before="RAC2P_equivalent_aapi_group10_household") %>% 
#  add_row(psrc_pums_count(df_pums_renter_all_race, group_vars=c("PRACE", "SOCP3")) %>%
#            mutate(RAC2P_equivalent_aapi_group10_household = "Asian", .after="COUNTY")) %>% 
# filter(PRACE == "Asian") %>% 
...
saveWorkbook(wb, "aapi_raw_data.xlsx", overwrite = TRUE)
