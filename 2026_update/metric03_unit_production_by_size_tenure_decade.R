...
export_path <- "J:/Projects/V2050/Housing/Monitoring/2026Update/data/metric03_units_in_structure_tenure"
source_info <- c("2024 American Community Survey (ACS) PUMS, 5-Year. Crunched by Eric Clute.")
...
pums <- pums_raw %>% 
  mutate(
    ...
    decade = factor(case_when(
      YRBLT >= 1980 & YRBLT <= 1989 ~ "1980s",
      YRBLT >= 1990 & YRBLT <= 1999 ~ "1990s",
      YRBLT >= 2000 & YRBLT <= 2009 ~ "2000s",
      YRBLT >= 2010 & YRBLT <= 2019 ~ "2010s",
      YRBLT >= 2020 & YRBLT <= year ~ "2020s",
