...
# Assumptions
setwd("J:/Projects/V2050/Housing/Monitoring/2026Update/data/metric03_units_in_structure_by_tenure")
years <- c(2010,2024) # base year and final year
compared_years <- c(2010,2016,2024) # comparison years over time
...
df_uis_owner_wide$rr_score_2024 <- (df_uis_owner_wide$moe_2024/1.645)/df_uis_owner_wide$estimate_2024*100
df_uis_owner_wide <- df_uis_owner_wide %>%
  mutate(rr_score_2010=factor(case_when(rr_score_2010 <= 15 ~"good",
         ...
         rr_score_2024=factor(case_when(rr_score_2024 <= 15 ~"good",
                                        rr_score_2024 <= 30 ~"fair",
                                        rr_score_2024 <= 50 ~"weak",
                                        rr_score_2024 > 50 ~"unreliable",
                                        !is.na(rr_score_2024) ~ NA)))

df_uis_renter_wide$rr_score_2010 <- (df_uis_renter_wide$moe_2010/1.645)/df_uis_renter_wide$estimate_2010*100
df_uis_renter_wide$rr_score_2024 <- (df_uis_renter_wide$moe_2024/1.645)/df_uis_renter_wide$estimate_2024*100
...
df_uis_owner_smalltbl <- pivot_wider(df_uis_owner_wide,id_cols = name, names_from = building_size, values_from = `growth_2010-24`)
df_uis_owner_smalltbl <- df_uis_owner_smalltbl %>% 
  select(name, `Single Family`, `2-19 units`, `20+ units`, `Mobile Home/Other`)

df_uis_renter_smalltbl <- pivot_wider(df_uis_renter_wide,id_cols = name, names_from = building_size, values_from = `growth_2010-24`)
...
saveWorkbook(work_book, file = "metric03_UIS_raw.xlsx", overwrite = TRUE)
